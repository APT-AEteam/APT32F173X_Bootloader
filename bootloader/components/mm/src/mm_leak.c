    ...
    while (cur) {
        if (!is_valid_address(cur)) {
            ...
            printf("!!!already corrupted after, stop traversaling!!!ptr=%lx size=%d pid=%d caller=%lx leak=%c\n",
                   (long unsigned int)(cur + 1), cur->size, cur->pid, (long unsigned int)cur->caller, cur->referenced ? 'N' : 'Y');
            break;
        }
...
static int show_chunk_snapshot(struct m_dbg_hdr *cur, void *unused)
{
    ...
    printf("ptr=%lx size=%d pid=%d caller=%lx\n",
           (long unsigned int)cur + 1, cur->size, cur->pid, (long unsigned int)cur->caller);
...
static int show_corrupted_chunk(struct m_dbg_hdr *cur, void *unused)
{
    ...
    printf("caller=%lx ptr=%lx size=%d  pid=%d leak=%c corrupted=%c\n",
           (long unsigned int)cur->caller, (long unsigned int)(cur + 1), cur->size, cur->pid,
           cur->referenced ? 'N' : 'Y', is_corrupted(cur) ? 'Y' : 'N');
...
void mm_leak_search_chunk(void *p)
{
    ...
    if (caddr) {
        printf("%s before : addr=%lx caller=%lx\n", __func__, (long unsigned int)caddr, (long unsigned int)caddr->caller);
    }

    if (caddr_l != (void *) - 1UL) {
        printf("%s after : addr=%lx caller=%lx\n", __func__, (long unsigned int)caddr_l, (long unsigned int)caddr_l->caller);
    }
}
