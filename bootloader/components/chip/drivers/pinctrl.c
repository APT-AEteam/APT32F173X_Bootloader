...
csi_error_t csi_pin_irq_mode(pin_name_e ePinName, csi_exi_grp_e eExiGrp, csi_gpio_irq_mode_e eTrgEdge)
{
	...
	ret = apt_gpio_intgroup_set(ptGpioBase,ePinName,(gpio_igrp_e)eExiGrp);		//interrupt group
	if(ret < 0)
		return CSI_ERROR;
...
csi_error_t csi_exi_set_flt(csi_exi_grp_e eExiGrp, csi_exi_flttm_e eFleTime)				
{
	...
	if(eExiGrp < EXI_GRP4)
	{
		csp_exi_flt(SYSCON, eExiGrp, (exi_flt_e)eFleTime);
	}
	else if(eExiGrp > EXI_GRP15)
	{
		csp_exi_flt(SYSCON, (eExiGrp - 12), (exi_flt_e)eFleTime);
	}
	else
		return CSI_ERROR;
